...
/**
 * Calculates the indegree of all nodes of a graph.
 *
 * @author Till Heistermann <till.heistermann@student.kit.edu>
 */
public class IndegreeCentrality extends CentralityImpl {
    /**
    ...
    public HashMap<Node, Double> getWeight(final Graph g) {
        ...
        if (g == null) {
            throw new IllegalArgumentException(
                    "Graph passed to Indegree is null.");
        }
        ...
        //increase the degree of each node by one for each incoming edge:
        for (Edge edge : g.getEdgeList()) {
            double degree = values.get(edge.getDestinationNode());
            values.put(edge.getDestinationNode(),
                       (degree + 1));
        }
        return values;
    }
